...
@section('auth_body')
    <form action="{{ $register_url }}" method="post">
        ...
        {{-- Legajo field --}}
        <div class="input-group mb-3">
            <input type="number" name="legajo" class="form-control {{ $errors->has('legajo') ? 'is-invalid' : '' }}"
                   value="{{ old('legajo') }}" placeholder="Legajo" autofocus>
            <div class="input-group-append">
                <div class="input-group-text">
                    <span class="fas fa-user {{ config('adminlte.classes_auth_icon', '') }}"></span>
                </div>
        ...
        {{-- Role --}}
        <div class="cinput-group mb-3">
                <select name="role_id"
                        id="role_id"
                        class="custom-select @error('role_id') is-invalid @enderror">
                            <option disabled selected value="">- Principal Actividad -</option>
